...
osThreadId TaskAnswerSysHandle;
osThreadId TaskGSMHandle;
osThreadId TaskESPHandle;
osThreadId TaskDS3231_AskHandle;
...
int main(void)
{
  ...
  MX_I2C1_Init();
  MX_SDIO_SD_Init();
	/* init code for FATFS (Truoc day no de o Defaut Task)*/
  MX_FATFS_Init();
  ...
  /* Create the thread(s) */
  /* definition and creation of TaskAnswerSys */
  osThreadDef(TaskAnswerSys, StartDefaultTask, osPriorityNormal, 0, 128);
  TaskAnswerSysHandle = osThreadCreate(osThread(TaskAnswerSys), NULL);

  /* definition and creation of TaskGSM */
  osThreadDef(TaskGSM, StartTask02, osPriorityNormal, 0, 128);
  TaskGSMHandle = osThreadCreate(osThread(TaskGSM), NULL);

  /* definition and creation of TaskESP */
  osThreadDef(TaskESP, StartTask03, osPriorityNormal, 0, 128);
  TaskESPHandle = osThreadCreate(osThread(TaskESP), NULL);

  /* definition and creation of TaskDS3231_Ask */
  osThreadDef(TaskDS3231_Ask, StartTask04, osPriorityNormal, 0, 128);
  TaskDS3231_AskHandle = osThreadCreate(osThread(TaskDS3231_Ask), NULL);

  /* definition and creation of TaskLCD */
  osThreadDef(TaskLCD, StartTask05, osPriorityNormal, 0, 128);
